...
// Main
(() => {
  const $$tagsDivs = document.querySelectorAll<HTMLDivElement>(
  ...
  for (const $tagDiv of $$tagsDivs) {
    ...
    const $$tags = $tagDiv.querySelectorAll<HTMLAnchorElement>(
      "li > span:last-child > a"
    );
...
function createComponent<K extends keyof HTMLElementTagNameMap>(
  tagName: K,
  options?: RecursivePartial<HTMLElementTagNameMap[K]>
) {
